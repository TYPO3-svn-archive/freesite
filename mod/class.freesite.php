...
class freesite_admin {
	...
	var $sysConfig = Array (
		...
		"uid_generalGroup" => 0,				// be_groups-UID, Optional: The user-group to use as a secondary group for all created users. New users will be a member of their own group + this group (if it exists). Good trick is to include this general group as a subgroup of the template group. This way new users to the site needs only being members of the template group and not both...
			// Create features:
		"copy_depth_level" => 3,				// Here you set the depth level of which Freesite should copy the pagetree of the dummy pages template. 
		...
		"alias_username" => 0,					// Boolean: If set, the alias field of the rootpage will be set to username.
...
Kind regards.
	...
	function genTree($theID, $depthData)	{
		...
		$res = mysql(TYPO3_db, "SELECT uid,title,doktype,hidden,deleted FROM pages WHERE pid = '$theID' AND NOT deleted ORDER BY sorting");
		$a=0;
	...
	function doesExist($table,$field,$value)	{
		$query="SELECT * FROM ".$table." WHERE ".$field."='".addslashes($value)."'";
	...
	function insertInDatabase($table,$data,$uid=0)	{
		if ($table && is_array($data))	{
			...
			if (count($fK))	{
				if ($uid)	{
					$uid=intval($uid);
					$query="UPDATE ".$table." SET ".implode($update,",")." WHERE uid=".$uid;
				} else {
					$query="INSERT INTO ".$table." (".implode($fK,",").") VALUES ('".implode($fV,"','")."')";
					$uid="999";
				}
			...
			while(list($k,$v)=each($this->data))	{
				...
				switch($k)	{
					case "FIELD_username":
						...
						$v=substr($v,0,20);
						if (strlen($v)<4 || $this->doesExist("be_users","username",$v))	{
	...
	function isReady()	{
		...
		if ($this->ALL_OK)	{
			...
			$VERIFICATION_CODE = strtoupper(trim($HTTP_POST_VARS["VERIFICATION_CODE"]));		// The sent ver-code
			if ($HTTP_POST_VARS["createButton"])		{	// Must press the createButton!
	...
	function enteringData()	{
		...
		if (!$this->ALL_OK || $HTTP_POST_VARS["updateButton"])	{
	...
	function displayDataForms()	{
		...
		$this->fieldsArray["DOMAIN_SECONDARY"]='<input type="Text" name="data[DOMAIN_SECONDARY]" value="'.$this->getInputData("DOMAIN_SECONDARY",1).'"'.$params.'>'.$this->isCorrect("DOMAIN_SECONDARY");
	...
	function previewData()	{
		$fields=explode(",","FIELD_realName,FIELD_email,FIELD_username,FIELD_password,FIELD_lang,TITLE,FOLDER,DOMAIN,DOMAIN_SECONDARY,DUMMY_PAGES,TEMPLATE");
...
//			}
	...
	function creatingData ()	{
		...
		$this->groupData=array();
			...
			// Copy template.
		$this->groupData = $this->groupTemplate;
...
//					$this->createTemplateRecord($root_page_pid,$this->getRootPageTitle());
		...
		//HOOK - afterSiteCreationPostProcess
		$_params = array('domainData'=>$domainData);
		if (is_array($GLOBALS['TYPO3_CONF_VARS']['EXTCONF']['freesite']['afterSiteCreationPostProcess']))	{
			foreach($GLOBALS['TYPO3_CONF_VARS']['EXTCONF']['freesite']['afterSiteCreationPostProcess'] as $funcRef)	{
	...
	function copyDummyPages($src_uid,$new_BE_USER)	{
		if (!$this->sysConfig["testingOnly"])	{
			...
			$tce->copyTree = $this->sysConfig['copy_depth_level'];
			...
			unset($new_BE_USER);
				...
				// Removal of sys_template and sys_domain records in root directory
			$removeRecordsFromRoot = array('sys_template'=>'(root!=0 OR clear!=0)','sys_domain'=>'');
			foreach($removeRecordsFromRoot as $removeRecordsFromRootTable => $removeRecordsFromRootAndWhere) {
				$deleteSysTemplateArray = $GLOBALS['TYPO3_DB']->exec_SELECTgetRows(
					'uid',
					$removeRecordsFromRootTable,
					'pid = '.$tce->copyMappingArray["pages"][$src_uid].($removeRecordsFromRootAndWhere!=''?' AND '.$removeRecordsFromRootAndWhere:'')
				);
				foreach($deleteSysTemplateArray as $deleteSysTemplateRow) {
					$res = $GLOBALS['TYPO3_DB']->exec_DELETEquery($removeRecordsFromRootTable,'uid='.$deleteSysTemplateRow['uid'].' AND pid='.$tce->copyMappingArray['pages'][$src_uid]);
				}
			}

				// Creating the main template record 
			$newTemplateUid=$this->createTemplateRecord($tce->copyMappingArray["pages"][$src_uid],$this->getRootPageTitle());
			
			// Translate uids
			//adding newly create records from be_users,be_groups and sys_template to the copyMappingArray 
 			$tempCopyMappingArray = $tce->copyMappingArray;
			if($this->user_uid) $tempCopyMappingArray['be_users'][] = $this->user_uid;
			if($this->userGroup_uid) $tempCopyMappingArray['be_groups'][] = $this->userGroup_uid;
			$tempCopyMappingArray['sys_template'][] = $newTemplateUid;
			// Array defines which fields should have uids translated.
			$fieldsToTranslate = array('sys_template'=>'config,constants','pages'=>'TSconfig','be_users'=>'TSconfig','be_groups'=>'TSconfig');// Der kopieres ikke be-brugere her:'be_users'=>'TSconfig','be_groups'=>'TSconfig' 
			foreach($fieldsToTranslate as $table => $fieldsList) {
				if($tempCopyMappingArray[$table]) {
					$res = $GLOBALS['TYPO3_DB']->exec_SELECTquery(
						'uid,'.$fieldsList,
						$table,
						'uid IN ('.implode(',',$tempCopyMappingArray[$table]).')'
					);
					$fieldsArray = explode(',',$fieldsList);
					while($row=$GLOBALS['TYPO3_DB']->sql_fetch_assoc($res)) {
						foreach($fieldsArray as $field)	{
							$sourceArray[$field]=explode("\n",preg_replace('/\r\n|\r/', "\n", $row[$field]));
							$sourceArrayTemp = $sourceArray[$field];
							$this->linesToRemove = array();
							array_walk($sourceArray[$field],array($this,'replaceUidArrayWalk'),$tce->copyMappingArray);
							foreach ($this->linesToRemove as $sourceArrayKeys) {
								unset($sourceArray[$field][$sourceArrayKeys]);
							}
							if($sourceArrayTemp != $sourceArray[$field]) {
								$rowUpdate[$field] = implode("\n",$sourceArray[$field]);
								$updateFlag=1;
							}
						}
						if($updateFlag==1){
							$resUpdate = $GLOBALS['TYPO3_DB']->exec_UPDATEquery($table,'uid='.$row['uid'],$rowUpdate);
							unset($updateFlag,$rowUpdate);
						}
						unset($sourceArray,$sourceArrayTemp);
					}
				}
			}
	...
	/**
	 * Function used to replace uids in values(string) of an array through the use of array_walk()
	 * ###NEW_UID_{TABLE}### 
	 */
	function replaceUidArrayWalk(&$item,$key,$copyMappingArray){
		$itemtemp = $item;
		if($this->tableToTranslateNextLine) {
			$table=$this->tableToTranslateNextLine;
			ereg('^.+ *= *([0-9,]+) *$',$item,$regs);
			if($regs[1]!="") { //uid found to translate
				if($table!="" AND is_array($copyMappingArray[strtolower($table)])) { //Table found in the copyMappingArray
			 		$replace = $copyMappingArray[strtolower($table)];
			 		$subject = explode(',',$regs[1]);
			 		foreach($subject as $value) {
			 			if(array_key_exists($value,$replace)) {
			 				$translatedUidsArray[]=$replace[$value];
			 			} elseif(($this->translateSettings['mode']=='keep' && $this->translateSettings['count'] > 0) OR ($this->translateSettings['mode']=='remove' && $this->translateSettings['count'] <= 0)) {
			 				$translatedUidsArray[]=$value;
			 			}
			 			$this->translateSettings['count']--;
			 		}
			 		$translatedUidsString = implode(',',$translatedUidsArray);
			 		if($translatedUidsString==$regs[1]){
			 			$comment = '##FREESITE: NO NEW '.$table.' RECORD UID(s) TO TRANSLATE!:';
			 		} else {
			 			$comment= '##FREESITE: '.$table.' RECORD UID(s) TRANSLATED.:';
			 		}
				} else { //The table is not found in the copyMappingArray
					$translatedUidsString = $regs[1];
					$comment= '##FREESITE: NO '.$table.' RECORDS WERE COPIED WHEN THE NEW SITE WAS CREATED!:';
				}
				$item=ereg_replace("^(.+ *= *)(".$regs[1].") *$",$comment."\n\\1".$translatedUidsString,$item);
			} else { // Nothing found to translate
//	 			$item=ereg_replace("^((.+))$","##FREESITE: NO UIDS TO TRANSLATE AT THE END OF THIS LINE!:\n\\2",$item);
	 			$item="##FREESITE: NO UIDS TO TRANSLATE AT THE END OF THIS LINE!:\n".$item;
	 		}
			unset($this->tableToTranslateNextLine);
		} else {
			ereg('^ *###NEW_UID_([a-zA-Z_]+)([+-]*)([0-9]*)### *$',$item,$findmarker);
			if($findmarker[1]) {
				$this->tableToTranslateNextLine=$findmarker[1];
				$this->linesToRemove[] = $key;
				if ($findmarker[2]) {
					$this->translateSettings['mode']=($findmarker[2]=='+'?'keep':'remove');
					if($findmarker[3]) {
						$this->translateSettings['count']=$findmarker[3];
					} else {
						$this->translateSettings['count']=10000;
					}
				} else {
					$this->translateSettings['mode']='keep';
					$this->translateSettings['count']=10000;
				}
			}
		}
	...
	function setRootPageProperties($root_page_pid)	{
		$pageData=array();
		if($this->sysConfig['alias_username']==1) $pageData["alias"]=$this->data["FIELD_username"];
	...
	function createDomainRecord($root_page_pid,$virtualDir)	{
		...
		if (($virtualDir || $this->data["DOMAIN"]) && $this->sysConfig["createDomains"])	{
			if ($this->data["DOMAIN_SECONDARY"]!='') $domainarray[] = $this->data["DOMAIN_SECONDARY"];
			if (!$this->data["DOMAIN"])	{
				$domainarray[] = $this->noDomainURL;
			} else {
				$domainarray[] = $this->data["DOMAIN"];
			}
			foreach ($domainarray as $domain) {
				$domain = $domain.'/';
				if ($virtualDir)	{	// Only prepend virtual dir if it's set!
					$domain.=$this->data["FOLDER"];
				}
				$domain = $this->rmSlash($domain);		// In any case, remove the last slash of the string.
				
				$domainData=array();
				$domainData["pid"]=$root_page_pid;
				$domainData["tstamp"]=time();
				$domainData["hidden"]=0;
				$domainData["domainName"]=$domain;
				$domainData["redirectTo"]="";
				
				if (!$this->doesExist("sys_domain","domainName",$domain))	{
	...
	function createTemplateRecord($root_page_pid,$title)	{
		if ($this->sysConfig["pid_templateArchive"])		{
			$this->templateRecordTemplate = $this->doesExist("sys_template","uid",$this->data["TEMPLATE"]);		// The users own group will be modelled after this group (be a copy)
			// ************************
			// Template data
			// ************************
			$templateData=array();
			$templateData=$this->templateRecordTemplate;
				// standard data
...
//			$templateData["basedOn"]=intval($this->data["TEMPLATE"]);
			...
			return $this->insertInDatabase("sys_template",$templateData);
		}
	}	
	...
	function printSelect($theType)	{
		...
		switch($theType)	{
			case "selectTemplate":
					// Template
				$infoUid = t3lib_div::GPvar("infoUid");
...
//					$out.='<HR><a href="index.php?script=template#T'.$infoUid.'">BACK</a>';
						...
						while($page_row=mysql_fetch_assoc($page_res))	{
							...
							while($row=mysql_fetch_assoc($res))	{
								...
								if (!$GLOBALS["TYPO3_CONF_VARS"]["BE"]["EXTERNAL"]["FREESITE"]["templateSelect_noInfoLink"])	{
									$out.='<a name="T'.$key.'"></a>';
									$out.='<a href="index.php?script=template&infoUid='.$key.'" target="template_infowindow"><img src="'.$GLOBALS["BACK_PATH"].'t3lib/gfx/icon_note.gif" align="absmiddle" width=18 height=16 border=0></a>';
								}
								$out.=$this->getTemplateHTML($key,$val,$itemPart);
							}
						}
	...
	function displayExample($theOutput)	{
		...
		$allDetails = t3lib_div::GPvar("allDetails");
